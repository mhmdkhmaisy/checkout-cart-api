...
@section('header', 'Create Wiki Page')

@section('content')
<div class="max-w-5xl">
    <form action="{{ route('admin.wiki.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-dragon-surface border border-dragon-border rounded-lg p-6">
            ...
            <div class="mt-4">
                ...
                <textarea name="description" rows="2"
                          class="w-full px-4 py-2 bg-dragon-accent border border-dragon-border rounded-lg text-dragon-silver focus:border-dragon-red focus:outline-none"
                          placeholder="Brief description (shown in search results)">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-dragon-surface border border-dragon-border rounded-lg p-6">
            ...
            <textarea name="content" id="wiki-editor" rows="20" required
                      ...
                      placeholder="Write your wiki content here...">{{ old('content') }}</textarea>
            @error('content')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex gap-4">
            <button type="submit" class="bg-dragon-red hover:bg-dragon-red-dark text-white px-6 py-3 rounded-lg transition-colors">
                <i class="fas fa-save mr-2"></i>Create Wiki Page
            </button>
...
<script src="https://cdn.tiny.cloud/1/ea6lmdx6nvv98d56f09lpj5uw8uamujpo66dpsll9w9c8hge/tinymce/8/tinymce.min.js" referrerpolicy="origin" crossorigin="anonymous"></script>
<script>
tinymce.init({
