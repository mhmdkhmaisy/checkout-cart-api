...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-dragon-silver-dark text-sm mt-1">Update revenue share settings for {{ $teamMember->name }}</p>
        </div>
    ...
    <div class="glass-effect rounded-xl border border-dragon-border p-6">
        <div class="mb-4 p-4 bg-dragon-surface rounded-lg border border-dragon-border">
            <div class="text-dragon-silver-dark text-sm">Current Total (excluding this member): <span class="text-dragon-gold font-bold">{{ number_format($currentTotal, 2) }}%</span></div>
            <div class="text-dragon-silver-dark text-sm">Maximum Allowed: <span class="text-green-400 font-bold">{{ number_format($maxAllowed, 2) }}%</span></div>
        </div>

        <form action="{{ route('admin.team-members.update', $teamMember) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="email" 
                       id="paypal_email" 
                       name="paypal_email" 
                       value="{{ old('paypal_email', $teamMember->paypal_email) }}"
                       required
                       class="w-full px-4 py-3 bg-dragon-surface border border-dragon-border rounded-lg text-dragon-silver focus:border-dragon-red focus:ring-1 focus:ring-dragon-red"
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" 
                           id="percentage" 
                           name="percentage" 
                           value="{{ old('percentage', $teamMember->percentage) }}"
                           required
                           step="0.01"
                           min="0.01"
                           max="{{ $maxAllowed }}"
            ...
            <div class="flex items-center">
                ...
                <input type="checkbox" 
                       ...
                       class="w-5 h-5 rounded border-dragon-border bg-dragon-surface text-dragon-red focus:ring-dragon-red">
                <label for="is_active" class="ml-3 text-sm text-dragon-silver">Active (receives payouts)</label>
            </div>
